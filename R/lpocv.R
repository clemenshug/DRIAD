...
## method - one of {"lgr", "svm", "xgb", "nn", "of"}
lpocv <- function( XY, lPairs, method="lgr" )
{
    if( !(method %in% c("lgr", "svm", "xgb", "nn", "of")) )
        stop( "method must be one of lgr, svm, xgb, nn", "of" )
    ...
    stopifnot(
        setequal(XY$Label, c("A", "B")) || (
            setequal(XY$Label, as.character(seq(0, 6))) && method == "of"
        )
    )
